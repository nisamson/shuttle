...
| `walk_record.py` | Prove the tail (`roster_end → record_end`) is fully self-delimiting: for each club it locates `roster_end` by data signature, then walks fixed gap1(219) + array1`[s4 N1][N1 s4]` + array2`[s4 N2][N2 s4]`+anchor + gap3(`managed?3607:2767`) + five_array`[s4 C][C×5-byte]` + gap4(65), and asserts the computed end lands byte-EXACT on the record boundary. Validated across all 9 records of both the AI-only and the human-managed reference save. This closes the last POST residual — the odd-width 5-byte-record array — and gives the validated length formula `record_end = roster_end + 219 + (4+4·N1) + (4+4·N2) + (managed?3607:2767) + (4+5·C) + 65`. Dropping `teams.ksy`'s `record_end_pos` offsets now only awaits decoding the record HEADER's `line_unit` (still a reference-save-only fixed view), not the tail. |
