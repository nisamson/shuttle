...
| `parse_teams.py` | Split `teams.dat` into team records from the data (no hard-coded offsets). Records carry no length prefix, so it anchors on the record-start signature — `unknown_1` = a sequential 0-based record index (0,1,2,…), then `team_id s4`, then the abbreviation/city/nickname QStrings — and reports each record's start, `team_id`, abbreviation, and size. Portable across saves; replaces `teams.ksy`'s reference-save-only `record_end_pos` offsets. |
