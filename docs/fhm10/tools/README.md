...
| `parse_teams.py` | Split `teams.dat` into team records from the data (no hard-coded offsets). Records carry no length prefix, so it anchors on the record-start signature — `unknown_1` = a sequential 0-based **record position** (0,1,2,…; distinct from `team_id`, which is a separate id and may be 0), then `team_id s4`, then the abbreviation/city/nickname QStrings — and reports each record's start, `team_id`, abbreviation, and size. Portable across clean saves; replaces `teams.ksy`'s reference-save-only `record_end_pos` offsets. Note `name` and `name_2` are separate fields that can diverge for relocated franchises, so it does not require them to be equal; its strict header filter may skip non-standard records in very large real-league saves. |
